...
<%
		int totalRecord = MUtil.parseInt(request, "totalRecord");
...
<table>
	<tr> 
		...
		<!-- 이전블럭 -->
		<%if(nowBlock>1){ %>
			<a href="javascript:sendBlock('<%=nowBlock-1%>')">prev...</a>
		<%}%>
		<!-- 페이징 -->
		<%
		...
		%>
			...
			<%if(pageStart==nowPage){ %><font color="blue"><b><%}%>
				[<%=pageStart%>]</a>		
			<%if(pageStart==nowPage){ %></b></font><%}%>
		<%}//--for%>
		<!-- 다음블럭 -->
		<%if(totalBlock>nowBlock){ %>
			<a href="javascript:sendBlock('<%=nowBlock+1%>')">...next</a>
		<%}%>
		</td>
	</tr>
</table>
