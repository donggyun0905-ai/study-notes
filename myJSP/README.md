...
### 2026-09-14 오후 (GuestBook 전체 완성 · 댓글 기능)

| 파일 | 내용 | 관련 챕터 |
|---|---|---|
| [guestbook_5_댓글과전체완성.md](./guestbook_5_댓글과전체완성.md) | 목록·글쓰기·수정·삭제·댓글 전체 완성본 분석, 가이드(4번)의 예상과 실제 코드가 갈린 지점(권한 검증 누락, 댓글 연쇄삭제 누락, grade 타입 통일) | guestbook |

### 2026-09-15 (ch14 회원가입/로그인/우편번호 검색)

| 파일 | 내용 | 관련 챕터 |
|---|---|---|
| [ch14_1_회원가입과로그인.md](./ch14_1_회원가입과로그인.md) | ch08 서블릿 기본기(GET/POST, 라이프사이클, 세션 로그인) 복습, ch14 회원가입·로그인·아이디 중복확인(팝업/AJAX)·우편번호 검색·취미 다중선택 인코딩, ch14a·ch15 예고 | ch08, ch14 |

### 2026-09-16 (ch15 계층형 게시판 — 목록/페이징/글읽기)

| 파일 | 내용 | 관련 챕터 |
|---|---|---|
| [ch15_1_계층형게시판.md](./ch15_1_계층형게시판.md) | `ref`/`pos`/`depth`로 답변 계층 표현하기, `LIMIT`으로 페이징, 올림 나눗셈으로 블럭 처리, 조회수 동시성 안전 증가(`count=count+1`), COS 파일첨부·본문타입(HTML/TEXT) 처리, ch15b(같은 로직 다른 화면) | ch15 |

### 2026-09-17 (ch15 공지사항 공식 구현 & 다크모드 토글)

| 파일 | 내용 | 관련 챕터 |
|---|---|---|
| [ch15_2_공지사항과다크모드.md](./ch15_2_공지사항과다크모드.md) | `mode` 컬럼으로 공지/일반글 분리, 공지 전용 조회 메소드 분리 설계, 체크박스 null 방어, `<head>`에서 동기적으로 먼저 테마 결정해 FOUC 막기, `<%@ include%>`로 다크모드 토글 공통화 | ch15 |

### 2026-09-18 (ch16 투표 프로그램)

| 파일 | 내용 | 관련 챕터 |
|---|---|---|
| [ch16_1_투표프로그램.md](./ch16_1_투표프로그램.md) | 설문(1):항목(N) 복합키 설계, 부모 먼저 저장 후 자식에 번호 물려주기, 체크박스/라디오를 `getParameterValues()`로 통일 처리, `num==0`="최신 설문" 관례, 0으로 나누기 방어, 비율 기반 막대그래프 시각화 | ch16 |
